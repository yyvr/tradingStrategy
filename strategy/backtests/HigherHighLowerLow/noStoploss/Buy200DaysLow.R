...
for ( i in 1:length(files) ) {
  ...
  while (j + maxHolding < nrow(stock)) {
    ...
    # past 200-day new low
    if (stock$close[j] <= min(stock$close[(j - 200) : j])) {
      returns <- NULL
...
write.csv(meta, 
          '/Users/yang/Downloads/invest/strategy/backtests/HigherHighLowerLow/noStoploss/Buy200DaysLow.csv')
